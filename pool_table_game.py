...
# coolant meter
coolant_flow_meter_pos = pg.Vector2(start_button_pos.x, (start_button_pos.y-25-coolant_flow_meter.get_height()))
coolant_flow_up_pos = pg.Vector2(coolant_flow_meter_pos.x-coolant_flow_up.get_width(),coolant_flow_meter_pos.y)
coolant_flow_down_pos = pg.Vector2(coolant_flow_meter_pos.x-coolant_flow_down.get_width(),coolant_flow_meter_pos.y+coolant_flow_up.get_height())
coolant_flow_meter_label_pos = pg.Vector2(coolant_flow_up_pos.x,coolant_flow_meter_pos.y-coolant_flow_meter_label.get_height())

# poison meter
poison_meter_pos = pg.Vector2(start_button_pos.x, (coolant_flow_meter_label_pos.y-25-poison_meter.get_height()))
poison_up_pos = pg.Vector2(coolant_flow_meter_pos.x-poison_up.get_width(),poison_meter_pos.y)
poison_down_pos = pg.Vector2(coolant_flow_meter_pos.x-poison_down.get_width(),poison_meter_pos.y+poison_up.get_height())
poison_meter_label_pos = pg.Vector2(poison_up_pos.x,poison_meter_pos.y-poison_meter_label.get_height())

game_over_message_pos =  pg.Vector2((screen.get_width()-game_over_message.get_width()) / 2, (screen.get_height()-game_over_message.get_height()) / 2)
...
def touching_rectrangle(surface1,position1,position2):
    return (position1.x < position2.x and position1.x + surface1.get_width() > position2.x and position1.y < position2.y and position1.y + surface1.get_height() > position2.y)
...
while running:
    ...
    for event in pg.event.get():
        if event.type == pg.QUIT:
            ...
            if touching_rectrangle(start_button,start_button_pos,click_pos) and rxn_started == False:
                rxn_started = True
                for i in range(50):
                    birth()
    ...
    add_text("criticality:" + str(0.01 * int(100*criticality)),pg.Vector2(screen.get_width() / 2, 10),screen,'black')
    add_text("operation time:" + str(int(t)) + " seconds",pg.Vector2(screen.get_width() -200, 10),screen,'black')
    ...
    add_text("^",pg.Vector2(poison_up.get_width() / 2, poison_up.get_height()/2),poison_up,'black')
    add_text("^",pg.Vector2(coolant_flow_up.get_width() / 2, coolant_flow_up.get_height()/2),coolant_flow_up,'black')
    add_text("v",pg.Vector2(poison_down.get_width() / 2, poison_down.get_height()/2),poison_down,'black')
    add_text("v",pg.Vector2(coolant_flow_down.get_width() / 2, coolant_flow_down.get_height()/2),coolant_flow_down,'black')
    add_text("poison insertion",pg.Vector2(poison_meter_label.get_width() / 2, poison_meter_label.get_height()/2),poison_meter_label,'black',fontsize = 15)
    add_text("coolant flow rate",pg.Vector2(coolant_flow_meter_label.get_width() / 2, coolant_flow_meter_label.get_height()/2),coolant_flow_meter_label,'black',fontsize = 15)
    ...
    for spot in poison_spots:
        poison_spot = pg.draw.circle(gameboard, poison_color, spot['position'], spot['size']*poison_effectiveness)
    ...
    for neutron in neutrons:    
        ...
        for poison in poison_spots:
            if touching_circle(neutron['position'],poison['position'],neutron_size,poison['size']*poison_effectiveness) and poison_effectiveness > 0:
                death(i)
        i += 1        
    ...
    while supercritical == True:
        ...
        add_text("Oh no! Your reactor has reached uncontrollable supercricitacality.", pg.Vector2(game_over_message.get_width()/2,game_over_message.get_height()/2-75),game_over_message,"black",25)
    ...
    while overheat == True:
        game_over_message.fill("yellow")
        add_text("Oh no! Your fuel has overheated.", pg.Vector2(game_over_message.get_width()/2,game_over_message.get_height()/2-75),game_over_message,"black",25)
        add_text("Total Operation Time: " + str(int(t)) + " seconds.", pg.Vector2(game_over_message.get_width()/2,game_over_message.get_height()/2-50),game_over_message,"black",25)
        add_text("Maximum Power Level: " + str(int(max_fissions*200)) + " MeV/second", pg.Vector2(game_over_message.get_width()/2,game_over_message.get_height()/2-25),game_over_message,"black",25)
        add_text("Game Over", pg.Vector2(game_over_message.get_width()/2, game_over_message.get_height()/2),game_over_message,"black",50)    
        add_text("Click anywhere to restart.", pg.Vector2( game_over_message.get_width()/2,game_over_message.get_height()/2+50),game_over_message,"black",25)
